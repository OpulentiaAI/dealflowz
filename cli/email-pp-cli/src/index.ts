...
// Includes a compound run-update-email command for Northcliff deal-flow process emails.
...
Run-update-email flags (compound):
  ...
  --stage="Primary filter"  Current CRM stage
  --decision="pass"       Decision: pass, review, fail, inactive, needs info
  --next-action="Call broker"  Next action description
...
Examples:
  ...
  email-pp-cli send --from=me@northcliff.com --to=team@northcliff.com --subject="Update" --body="Status: good"
  email-pp-cli draft --from=me@northcliff.com --to=team@northcliff.com --subject="Draft" --body="Review needed"
  email-pp-cli messages --query="from:broker@ib.com" --max-results=5
  email-pp-cli message --id=abc123
  email-pp-cli labels
  email-pp-cli run-update-email --from=me@northcliff.com --to=team@northcliff.com --company="Acme LLC" --stage="Primary filter" --decision=pass --next-action="Call broker" --work="Grata search,Attio update" --verifications="Attio:record created,Email:verified"
  email-pp-cli run-update-email --from=me@northcliff.com --to=team@northcliff.com --company="Acme LLC" --stage="Review" --decision=review --draft
`;
...
// Compound: build a Northcliff process-run update email and send or draft it.
async function cmdRunUpdateEmail(flags: Record<string, string | boolean>): Promise<void> {
  ...
  const subject = getFlagStr(flags, "subject") ?? `Northcliff deal-flow update: ${company} - ${decision}`;

  // Build email body following the process-run-email.md template
  const bodyLines: string[] = [];
  bodyLines.push(`Hi,`);
  bodyLines.push("");
  bodyLines.push(`Here is the update from the latest Northcliff deal-flow run.`);
  bodyLines.push("");
