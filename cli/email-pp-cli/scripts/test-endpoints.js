...
test("send: POST /users/me/messages/send with base64url raw body", () => {
  const r = runWithMock(["send", "--from=me@northcliff.com", "--to=team@northcliff.com", "--subject=Test", "--body=Hello"]);
  assert(r.ok, `should exit 0, err: ${r.err}`);
  ...
  assert(decoded.includes("From: me@northcliff.com"), "should have From header");
  assert(decoded.includes("To: team@northcliff.com"), "should have To header");
...
test("draft: POST /users/me/drafts with message raw", () => {
  const r = runWithMock(["draft", "--from=me@northcliff.com", "--to=team@northcliff.com", "--subject=Draft", "--body=Review"]);
  assert(r.ok, `should exit 0, err: ${r.err}`);
...
test("run-update-email: sends email with correct body structure", () => {
  ...
  const body = JSON.parse(r.calls[0].body);
  const decoded = Buffer.from(body.raw.replace(/-/g, "+").replace(/_/g, "/"), "base64").toString("utf-8");
  assert(decoded.includes("Northcliff deal-flow update: Acme LLC - pass"), "should have auto-generated subject");
  assert(decoded.includes("Stage: Primary filter"), "should have stage in body");
  assert(decoded.includes("Decision: pass"), "should have decision in body");
  assert(decoded.includes("Next action: Call broker"), "should have next action in body");
  assert(decoded.includes("Grata search"), "should have work item in body");
