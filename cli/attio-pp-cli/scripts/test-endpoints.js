...
test("create-note: POST /v2/notes with title and content", () => {
  const r = runWithMock(["create-note", "--parent-object=companies", "--parent-record-id=rec_001", "--title=Source", "--content=Grata search"]);
  assert(r.ok, `should exit 0, err: ${r.err}`);
  assertCall(r.calls[0], {
    urlEquals: "https://api.attio.com/v2/notes",
    method: "POST",
    authBearer: true,
    bodyContains: { parent_object: "companies", parent_record_id: "rec_001", title: "Source", content: "Grata search" },
  });
});
